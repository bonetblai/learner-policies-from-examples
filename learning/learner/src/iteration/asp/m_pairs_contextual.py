...
class MPairsContextual:
    def __init__(self, ds: Dict[str, Any], monotone_only_by_dec: bool = False, lazy: bool = False, timers: Optional[Statistics] = None):
        ...
        self._ext_states_to_contexts: Dict[Tuple[int, int], Set[Tuple[int, int]]] = defaultdict(set)
    ...
    def _initialize(self, lazy: bool):
        ...
        #XXX self._m_pair_index_to_g_idxs: List[intbitset] = []
        self._m_pair_index_to_f_idxs: List[Tuple[intbitset, intbitset]] = []
        self._g_idx_to_m_pair_index: Dict[int, int] = dict()
        #XXX self._f_idx_to_m_pair_indices: Dict[int, intbitset] = defaultdict(intbitset)

        self._monotone_features: intbitset = None
        self._usable_features: intbitset = None
        #XXX self._fg_companions: Dict[int, intbitset] = defaultdict(intbitset)
        self._gf_companions: Dict[int, Tuple[intbitset, intbitset]] = dict()
    ...
    def _m_pair_index_for_g_idx(self, g_idx: int) -> int:
        ...
        if m_pair_index is None:
            ...
            if m_pair_index is None:
                ...
                #XXX self._m_pair_index_to_g_idxs.append(intbitset())
                self._m_pair_index_to_f_idxs.append(None)
            #XXX self._m_pair_index_to_g_idxs[m_pair_index].add(g_idx)
            self._g_idx_to_m_pair_index[g_idx] = m_pair_index
        return m_pair_index

    def _f_idxs_for_m_pair_index(self, m_pair_index: int) -> Tuple[intbitset, intbitset]:
        f_idxs: Tuple[intbitset, intbitset] = self._m_pair_index_to_f_idxs[m_pair_index]
        if f_idxs is None:
            # m_pair[0] (resp. m_pair[1]) for g_idx is set of ext-states where g_idx remains "eqv" and g_idx Boolean value is 0 (resp. 1)
            ...
            for bvalue, m in enumerate(m_pair):
                if len(m) == 0:
                    # No rules, every f_idx is monotone
                    bvalue_to_f_idxs.append(intbitset(self._relevant_features_idxs))
                else:
                    # f_idx is monotone if it can only change in one direction
            ...
            f_idxs: Tuple[intbitset, intbitset] = (bvalue_to_f_idxs[0], bvalue_to_f_idxs[1])
            self._m_pair_index_to_f_idxs[m_pair_index] = f_idxs
            #XXX for f_idx in f_idxs: self._f_idx_to_m_pair_indices[f_idx].add(m_pair_index)
        return f_idxs
    ...
    def _calculate_monotone_features(self) -> intbitset:
        if self._monotone_features is None:
            ...
            logging.info(f"MPairsContextual: {len(self._monotone_features)} monotone feature(s) calculated in {local_timer.get_elapsed_sec():.02f} second(s)")
    ...
    def _calculate_usable_features(self) -> intbitset:
        if self._usable_features is None:
            ...
            while len(new_usable_features) > 0:
                ...
                f_idxs_pair: Tuple[intbitset, intbitset] = self.f_idxs_for_g_idxs(self._usable_features)
                new_usable_features: intbitset = (f_idxs_pair[0] | f_idxs_pair[1]) - self._usable_features
    ...
    def _calculate(self, calculate_fg_and_gf_companions: bool = False):
        ...
        logging.info(f"MPairsContextual: #features={len(self._relevant_features_idxs)}, #m-pairs={len(self._m_pair_index_to_m_pair)}")

        # Calculate f_idxs enabled by each context
        ...
        logging.info(f"MPairsContextual: maps for f_idxs and g_idxs computed in {local_timer.get_elapsed_sec():.02f} second(s)")
        ...
        if calculate_fg_and_gf_companions:
            ...
            local_timer.stop()
            self._timers.stop("m_pairs")
            logging.info(f"MPairsContextual: #fg={sum([len(g_idxs) for f_idx, g_idxs in self._fg_companions.items()])} and #gf={sum([len(f_idxs) for g_idx, f_idxs in self._gf_companions.items()])} companion pair(s) computed in {local_timer.get_elapsed_sec():.02f} second(s)")
        """
    ...
    def f_idxs_for_g_idx(self, g_idx: int) -> Tuple[intbitset, intbitset]:
        f_idxs: Tuple[intbitset, intbitset] = self._gf_companions.get(g_idx)
        if f_idxs is None:
            m_pair_index: int = self._m_pair_index_for_g_idx(g_idx)
            f_idxs: Tuple[intbitset, intbitset] = self._f_idxs_for_m_pair_index(m_pair_index)
            f_idxs: Tuple[intbitset, intbitset] = (f_idxs[0] - self._monotone_features - intbitset([g_idx]), f_idxs[1] - self._monotone_features - intbitset([g_idx]))
            self._gf_companions[g_idx] = f_idxs
        return f_idxs

    def f_idxs_for_g_idxs(self, g_idxs: intbitset) -> Tuple[intbitset, intbitset]:
        f_idxs: List[intbitset, intbitset] = [intbitset(self._monotone_features), intbitset(self._monotone_features)]
        for g_idx in g_idxs:
            f_idxs_pair: Tuple[intbitset, intbitset] = self.f_idxs_for_g_idx(g_idx)
            f_idxs[0] |= f_idxs_pair[0]
            f_idxs[1] |= f_idxs_pair[1]
        return tuple(f_idxs)

...
def make_conditions(policy_builder: dlplan_policy.PolicyFactory,
                    ...
                    feature_valuations: array,
                    f_idxs: Optional[List[int]] = None) -> Set[Any]:
    ...
    items = enumerate(zip(feature_pool, feature_valuations)) if f_idxs is None else [(f_idx, (feature_pool[f_idx], feature_valuations[f_idx])) for f_idx in f_idxs]
    for f_idx, (feature, val) in items:
...
def make_effects(policy_builder: dlplan_policy.PolicyFactory,
                 feature_pool: List[Feature],
                 source_feature_valuations: array,
                 target_feature_valuations: array,
                 f_idxs: Optional[List[int]] = None) -> Set[Any]:
    ...
    items = enumerate(zip(feature_pool, source_feature_valuations, target_feature_valuations)) if f_idxs is None else [(f_idx, (feature_pool[f_idx], source_feature_valuations[f_idx], target_feature_valuations[f_idx])) for f_idx in f_idxs]
    for f_idx, (feature, source_val, target_val) in items:
        if isinstance(feature.dlplan_feature, dlplan_core.Boolean):
...
def compute_state_pair_equivalences(preprocessing_data: PreprocessingData,
    ...
    rules = []
    rule_repr_to_idx: Dict[str, int]  = dict()

    gfa_state_id_to_state_pair_equivalence: Dict[int, StatePairEquivalence] = dict()
    gfa_state_global_idx_to_instance_and_gfa_state_idx: Dict[int, Tuple[int, int]] = dict()
    instance_idx_to_rule_repr: Dict[int, Set(str)] = defaultdict(set)
    ...
    for gfa_state in iteration_data.gfa_states:
        ...
        gfa_state_idx = gfa_state.get_faithful_abstract_state_index()
        gfa_state_global_idx = gfa_state.get_global_index()
        gfa_state_global_idx_to_instance_and_gfa_state_idx[gfa_state_global_idx] = (instance_idx, gfa_state_idx)
        ...
        for s_distance, mimir_ss_states_prime in enumerate(tuple_graph_states_by_distance):
            for mimir_ss_state_prime in mimir_ss_states_prime:
                ...
                instance_idx_to_rule_repr[instance_idx].add(rule_repr)
    ...
    return rules, gfa_state_id_to_state_pair_equivalence, gfa_state_global_idx_to_instance_and_gfa_state_idx, instance_idx_to_rule_repr, rule_repr_to_idx

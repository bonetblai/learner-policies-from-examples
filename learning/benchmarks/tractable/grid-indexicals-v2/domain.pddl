...
(:action FIRST-MARK
  :parameters (?key)
  :precondition (and (key ?key) (no-marks))
  :effect (and (not (no-marks)) (marked ?key)))

(:action MOVE-MARK
  :parameters (?key1 ?key2)
  :precondition (and (key ?key1) (key ?key2) (marked ?key1))
  :effect (and (not (marked ?key1)) (marked ?key2)))
)

...
(define (domain TIDYBOT)
  ...
  (:types robot cart myobject xc yc xrel yrel)
  (:predicates
   ;; Constant preds
   (leftof                   ?x1 - xc ?x2 - xc)
   (above                    ?y1 - yc ?y2 - yc)
   ...
   (zerox-rel    ?x - xrel)
   (zeroy-rel    ?y - yrel)
   (object-goal  ?o - myobject ?x - xc  ?y - yc)
   ...
   ;; Objects
   (object-pos   ?o - myobject ?x - xc  ?y - yc)
   (object-done  ?o - myobject)
   (surface ?x - xc ?y - yc)

   ;; Gripper
   (holding       ?r - robot ?o - myobject)
  ...
  (:action put-on-cart
   :parameters (?r - robot ?x - xc ?y - yc ?gxrel - xrel 
                ?gyrel - yrel ?o - myobject ?c - cart ?cx - xc ?cy - yc)
  ...
  (:action finish-object
   :parameters (?o - myobject ?x - xc ?y - yc)

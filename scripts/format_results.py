...
def read_data(filepath: Path) -> Dict[str, Any]:
    assert "slurm.output" in filepath.name
    ...
    parsing: str = None
    data: Dict[str, Any] = {
        "domain": filepath.name[:filepath.name.find(".slurm")],
...
if __name__ == "__main__":
    ...
    output_files: List[Path] = [filepath for folder_path in folder_paths for filepath in folder_path.glob("*") if ".slurm.output." in filepath.name]
    ...
    if not args.only_failures:
        ...
        sorted_selected_datas: List[Dict[str, Any]] = sorted([data for data in selected_datas if data.get("domain") not in blacklist], key=key)
